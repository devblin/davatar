const { checkBackgroundColor } = require('../../validationFunctions')

const { INVALID_BACKGROUND_COLOR } = require('../../errorConstants')
...
describe('Check background color', () => {
	it.each([
		...
		({ input, expected }) => {
			...
			expect(checkBackgroundColor(input)).toBe(expected)

			if (!expected) {
				expect(console.log.mock.calls.length).toBe(1)

				expect(console.log.mock.calls[0][0]).toBe(
					`Davatar: ${INVALID_BACKGROUND_COLOR}`
				)
			} else {
